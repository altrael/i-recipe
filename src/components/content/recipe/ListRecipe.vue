  ...
  <div class="flex flex-col justify-center items-center w-3/4">
    <div class="p-7 rounded-xl border border-gray-500 w-full">
      <div class="mb-3 font-bold flex items-center">
        ...
        />
        <button @input="$emit('updateTitle', title)" class="btn btn-warning btn-sm">Update Title</button>
      </div>
        ...
        <div v-for="(step, index) in data.steps" :key="index" class="flex flex-col mb-3">
          ...
          <template v-if="step.type === 'text'">
            <div class="flex flex-col md:flex-row">
              <input type="text" placeholder="Step" class="input input-bordered input-sm md:input-md grow mb-3 md:me-3" />
              <button class="btn btn-outline btn-error btn-sm md:btn-md" @click="$emit('deleteStep', index)">Delete Step</button>
            </div>
          </template>
          <div v-else-if="step.type === 'textarea'" class="flex flex-col md:flex-row md:items-center">
            <textarea placeholder="Description" class="textarea textarea-bordered textarea-sm md:textarea-md grow mb-3 md:me-3" rows="3"></textarea>
            <button class="btn btn-outline btn-error btn-sm md:btn-md" @click="$emit('deleteStep', index)">Delete Step</button>
          </div>
